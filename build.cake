...
var xunit = "test/Nancy.Bootstrappers.Unity.Tests/bin/"+configuration+"/net452/unix-x64/dotnet-test-xunit.exe";
  ...
  .Does(() =>
{
  ...
  else 
  {
    ...
    using(var process = StartAndReturnProcess("mono", new ProcessSettings{Arguments = 
      xunit + " " + 
      "test/Nancy.Bootstrappers.Unity.Tests/bin/"+configuration+"/net452/unix-x64/Nancy.Bootstrappers.Unity.Tests.dll"}))
    {
      process.WaitForExit();
      if(process.GetExitCode() != 0)
      {
        throw new Exception("Nancy.Bootstrappers.Unity.Tests failed");
      }
    }
  }
});
...
Task("Package")
  ...
  .Does(() =>
{
  var package = outputPackages + File("Nancy.Bootstrappers.Unity-Latest.zip");
